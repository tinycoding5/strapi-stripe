...
const Configuration = () => {
  ...
  const handleChange = event => {
    ...
    } else if (name === 'accountLinkReturnUrl') {
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      !stripeConfiguration.accountLinkReturnUrl &&
      !stripeConfiguration.accountLinkRefreshUrl
    ) {
      setError({
        ...error,
        ...
        accountLinkReturnUrl: 'Account Onboarding return URL is required',
      ...
      setIsSubmitting(false);
    } else if (!stripeConfiguration.accountLinkReturnUrl) {
      setError({
        ...error,
        accountLinkReturnUrl: 'Account Onboarding Return URL is required',
      });
      setIsSubmitting(false);
    } else if (!stripeConfiguration.accountLinkRefreshUrl) {
      setError({
        ...error,
        accountLinkRefreshUrl: 'Account Onboarding Refresh URL is required',
      });
            ...
            <Grid gap={4}>
              <GridItem col={6} s={12}>
                <Box paddingTop={2} paddingBottom={2}>
                  <TextInput
                    name="accountLinkReturnUrl"
                    label="Account Onboarding Return URL"
                    required
                    value={stripeConfiguration.accountLinkReturnUrl}
                    error={error.accountLinkReturnUrl ? error.accountLinkReturnUrl : ''}
                    onChange={handleChange}
                    hint="Redirects to the return page after the onboarding successful"
                  />
                </Box>
              </GridItem>
              <GridItem col={6} s={12}>
                <Box paddingTop={2} paddingBottom={2}>
                  <TextInput
                    name="accountLinkRefreshUrl"
                    label="Account Onboarding Refresh URL"
                    required
                    value={stripeConfiguration.accountLinkRefreshUrl}
                    error={error.accountLinkRefreshUrl ? error.accountLinkRefreshUrl : ''}
                    onChange={handleChange}
                    hint="Refresh again after the  onboarding failed"
                  />
                </Box>
              </GridItem>
              <GridItem col={6} s={12}>
